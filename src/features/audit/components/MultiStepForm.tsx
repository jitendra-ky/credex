...
import { ArrowRight, Check, Code, Bot, MessagesSquare, Sparkles, Terminal } from 'lucide-react';
...
const AVAILABLE_TOOLS = [
  { id: 'cursor', name: 'Cursor', icon: Terminal, color: 'bg-black' },
  { id: 'claude_gui', name: 'Claude', icon: Bot, color: 'bg-[#D97757]' },
  { id: 'chatgpt_gui', name: 'ChatGPT', icon: MessagesSquare, color: 'bg-[#10A37F]' },
  { id: 'gemini', name: 'Gemini', icon: Sparkles, color: 'bg-blue-500' },
  { id: 'v0_vercel', name: 'v0 (Vercel)', icon: Code, color: 'bg-black' },
];
...
export function MultiStepForm({ onSubmit }: MultiStepFormProps) {
  ...
  const handleSubmit = () => {
    // Transform simplified state to ValidatedAuditRequest
    const payload: any = {
      global_context: {
    ...
    // Fill in active tools
    selectedTools.forEach(toolId => {
      const details = toolDetails[toolId] || {};
      const defaultPlan = TOOL_PLAN_OPTIONS[toolId]?.[1]?.value || 'pro'; // Default to second option (first paid plan)
